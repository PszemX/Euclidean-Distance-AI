...
class HiddenLayer:
    def __init__(self, input_size, output_size, neurons, activation):
        ...
        self.output_size = np.ones(output_size)
    
    def forward(self, X):
        self.activation.forward(np.dot(X, self.weight) + self.bias)
        self.hidden = self.activation.output
        self.output = np.dot(self.output, self.output_size)
        return self.output
