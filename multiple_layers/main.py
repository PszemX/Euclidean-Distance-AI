...
def configure_model(model, input):
    # Add layers
    model.add(Layer(input_size=input, output_size=128, activation=ActivationReLU()))
    ...
    model.configureTraining(epochs=100, batch_size=64, clip_threshold=5.0, accuracy=0.1, learning_rate_change_frequency=10,
...
def sub_model():
    # Generate data
    generator = Generator(train_percentage='80%', dimensions=2)
    x_train, y_train, x_test, y_test = generator.generate(size=1000, min_range=1, max_range=100, type='substractionAB')
    # Initialize model
    model = NeuralNetwork()
    configure_model(model=model, input=2)
    # Train model
